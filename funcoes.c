...
void *serv_cli(void *dados)
{
	...
	int r, w, i, c_fifo_fd, ida[2], volta[2];
	;
	char c_fifo_fname[20], total[1000];
	pid_t processo;
	...
	{ //Se for filho
		...
		else
		{
			if (pipe(ida) < 0)
			{
				...
				com.request.texto[0] = '&';
				sprintf(c_fifo_fname, CLIENT_FIFO, com.request.pid_cliente);
				write(c_fifo_fd, &com.request, sizeof(com.request));
			}
			if (pipe(volta) < 0)
			{
			...
			{ // se for pai
				...
				strcpy(com.request.texto, total);
				...
				else
				{
					...
					w = write(c_fifo_fd, &com.request, sizeof(com.request));
