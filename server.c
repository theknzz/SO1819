...
int main(int argc, char **argv)
{
	...
	// Dá destino ao recebimento de um sinal SIGUSR1
	...
	// preenche o nome do pipe com quem vai falar
	sprintf(c_fifo_fname, CLIENT_FIFO, val.pid_user);

	// abre o npipe para WR
	...
	// de validacao
	w = write(c_fifo_fd, &val, sizeof(val));
	if (w == sizeof(val))
	...
	while (1)
	{
		...
		else
		{
			...
			w = write(c_fifo_fd, &req, sizeof(req));
			if (w == sizeof(req))
				fprintf(stderr, "\nescreveu a resposta");
			else
